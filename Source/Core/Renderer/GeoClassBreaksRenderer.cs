...
namespace DEETU.Core
{
    public class GeoClassBreaksRenderer : GeoRenderer
    {
        ...
        private List<GeoSymbol> _Symbols = new List<GeoSymbol>();//符号集合，小于第一个分割值对应第一个符号，依此类推
        ...
        public override GeoRendererTypeConstant RendererType
        {
            get
            {
                return GeoRendererTypeConstant.ClassBreaks;
            }
        }
        ...
        /// <param name="index"></param>
        /// <returns></returns>
        public GeoSymbol GetSymbol(Int32 index)
        {
        ...
        /// <param name="index"></param>
        /// <param name="value"></param>
        public void SetSymbol(Int32 index, GeoSymbol symbol)
        {
        ...
        public void AddBreakValue(double value, GeoSymbol symbol)
        {
        ...
        public void AddBreakValues(double[] values, GeoSymbol[] symbols)
        {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public GeoSymbol FindSymbol(double value)
        {
        ...
        public void RampColor(Color startColor, Color endColor)
        {
            ...
            for (Int32 i = 0; i <= sBreakCount - 1; i++)
            {
                if (_Symbols[i] != null)
                {
                    if (_Symbols[i].SymbolType == GeoSymbolTypeConstant.SimpleMarkerSymbol)
                    {
                        GeoSimpleMarkerSymbol sSymbol = (GeoSimpleMarkerSymbol)_Symbols[i];
                        sSymbol.Color = sColors[i];
                    }
                    else if (_Symbols[i].SymbolType == GeoSymbolTypeConstant.SimpleLineSymbol)
                    {
                        GeoSimpleLineSymbol sSymbol = (GeoSimpleLineSymbol)_Symbols[i];
                        sSymbol.Color = sColors[i];
                    }
                    else if (_Symbols[i].SymbolType == GeoSymbolTypeConstant.SimpleFillSymbol)
                    {
                        GeoSimpleFillSymbol sSymbol = (GeoSimpleFillSymbol)_Symbols[i];
                        sSymbol.Color = sColors[i];
                    }
                }
            }
