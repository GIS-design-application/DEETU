...
namespace DEETU.Map
{
    public static class GeoMapTools
    {
        #region 基本部分
        ...
        internal static bool AreBoxesCross(GeoRectangle box1, GeoRectangle box2)
        {
        ...
        /// <returns></returns>
        internal static bool IsRayCrossSegment(GeoPoint point, GeoPoint sPoint, GeoPoint ePoint)
        {
        ...
        /// <param name="ePoint"></param>
        /// <returns></returns>
        internal static bool IsScanCrossSegment(double scanY, GeoPoint sPoint, GeoPoint ePoint)
        {
            if (sPoint.Y == ePoint.Y)
                //与扫描线平行
                return false;
        ...
        /// <returns></returns>
        internal static Int32 GetIntersectionCountBetweenRayAndPolygon(GeoPoint point, GeoPoints points)
        {
        ...
        /// <returns></returns>
        internal static List<double> GetIntersectionsBetweenScanAndPolygon(double scanY, GeoPoints points)
        {
            Int32 sPointCount = points.Count;
        ...
        /// <returns></returns>
        internal static bool AreSegmentsCross(GeoPoint sPoint1, GeoPoint ePoint1, GeoPoint sPoint2, GeoPoint ePoint2)
        {
            //思路：采用参数方程求解，原理如下
        ...
        /// <returns></returns>
        internal static bool IsSegmentCrossBox(GeoPoint sPoint, GeoPoint ePoint, GeoRectangle box)
        {
            ...
            if (sPoint.Y > box.MaxY && ePoint.Y > box.MaxY)
                ...
                return false;
            //定义四个顶点
            GeoPoint sPoint1 = new GeoPoint(box.MinX, box.MaxY);           //矩形左上点
            GeoPoint sPoint2 = new GeoPoint(box.MaxX, box.MaxY);          //矩形右上点
            GeoPoint sPoint3 = new GeoPoint(box.MaxX, box.MinY);          //矩形右下点
            GeoPoint sPoint4 = new GeoPoint(box.MinX, box.MinY);          //矩形左下点
            //将每条边与线段求交
        ...
        /// <param name="points"></param>
        /// <returns></returns>
        internal static double GetPolygonSquare(GeoPoints points)
        {
            Int32 sPointCount = points.Count;
            double s = 0;
        ...
        /// <returns></returns>
        public static bool IsPointOnPoint(GeoPoint point, GeoPoint pointOverlapped, double tolerance)
        {
        ...
        /// <param name="tolerance"></param>
        /// <returns></returns>
        public static bool IsPointOnPolyline(GeoPoint point, GeoPoints points, double tolerance)
        {
        ...
        /// <param name="tolerance"></param>
        /// <returns></returns>
        public static bool IsPointOnMultiPolyline(GeoPoint point, GeoMultiPolyline multiPolyline, double tolerance)
        {
            GeoRectangle sBox = new GeoRectangle(multiPolyline.MinX - tolerance, multiPolyline.MaxX + tolerance,
        ...
        /// <param name="box"></param>
        /// <returns></returns>
        public static bool IsPointWithinBox(GeoPoint point, GeoRectangle box)
        {
        ...
        /// <returns></returns>
        public static bool IsPointWithinMultiPolygon(GeoPoint point, GeoMultiPolygon multiPolygon)
        {
            //（1）判断点是否位于外包矩形内，如否则返回否
            GeoRectangle sExtent = multiPolygon.GetEnvelope();
        ...
        /// <param name="box"></param>
        /// <returns></returns>
        public static bool IsMultiPolylinePartiallyWithinBox(GeoMultiPolyline multipolyline, GeoRectangle box)
        {
            //思路：先判断矩形盒是否相交，如是，按如下顺序，满足任何一个条件，则返回True
            //（1）复合折线任何一个点位于矩形盒内;
            //（2）矩形盒与复合折线有交点
            GeoRectangle sBox = multipolyline.GetEnvelope();
            ...
            for (Int32 i = 0; i <= sPartCount - 1; i++)
            {
                GeoPoints sPoints = multipolyline.Parts.GetItem(i);
        ...
        /// <param name="box"></param>
        /// <returns></returns>
        public static bool IsMultiPolygonPartiallyWithinBox(GeoMultiPolygon multipolygon, GeoRectangle box)
        {
            //思路：先判断矩形盒是否相交，如是，按如下顺序，满足任何一个条件，则返回True
            //（1）复合多边形任何一个点位于矩形盒内;
            //（2）矩形盒任何一个顶点位于复合多边形内
            //（3）矩形盒与复合多边形有交点
            GeoRectangle sBox = multipolygon.GetEnvelope();
            if (AreBoxesCross(sBox, box) == false)
                return false;
            //（1）多边形任何一个点位于矩形盒内
            Int32 sPartCount = multipolygon.Parts.Count;
            ...
            //（2）矩形盒任何一个顶点位于多边形内
            GeoPoint sRectPoint = new GeoPoint(box.MinX, box.MinY);   //左下点
            if (IsPointWithinMultiPolygon(sRectPoint, multipolygon) == true)
                return true;
            sRectPoint = new GeoPoint(box.MinX, box.MaxY);           //左上点
            if (IsPointWithinMultiPolygon(sRectPoint, multipolygon) == true)
                return true;
            sRectPoint = new GeoPoint(box.MaxX, box.MaxY);           //右上点
            if (IsPointWithinMultiPolygon(sRectPoint, multipolygon) == true)
                return true;
            sRectPoint = new GeoPoint(box.MaxX, box.MinY);           //右下点
            if (IsPointWithinMultiPolygon(sRectPoint, multipolygon) == true)
                return true;
            //（3）矩形盒与复合多边形有交点
            for (Int32 i = 0; i <= sPartCount - 1; i++)
            {
                GeoPoints sPoints = multipolygon.Parts.GetItem(i);
                Int32 sPointCount = sPoints.Count;
                for (Int32 j = 0; j <= sPointCount - 2; j++)
                {
                    if (IsSegmentCrossBox(sPoints.GetItem(j), sPoints.GetItem(j + 1), box) == true)
        ...
        /// <param name="points"></param>
        /// <returns></returns>
        public static GeoPoint GetMidPointOfPolyline(GeoPoints points)
        {
            Int32 sPointCount = points.Count;
            ...
            GeoPoint sMidPoint = new GeoPoint(x, y);
            return sMidPoint;
        }
        ...
        /// <param name="multiPolygon"></param>
        /// <returns></returns>
        public static GeoPoint GetLabelPointOfMultiPolygon(GeoMultiPolygon multiPolygon)
        {
            ...
            GeoPoint sLabelPoint = new GeoPoint(sLabelPointX, sScanY);
            return sLabelPoint;
        }
        #endregion
